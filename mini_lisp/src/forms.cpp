...
const std::unordered_map<std::string, SpecialFormType *> SPECIAL_FORMS{//管理特殊形式的map
        {"define",     &defineForm},
        {"if",         &ifForm},
        {"and",        &andForm},
        {"or",         &orForm},
...
void checkMinSize(const std::vector<ValuePtr> &params, int size, const std::string &name);//用于检查参数个数

void checkExactSize(const std::vector<ValuePtr> &params, int size, const std::string &name);//用于检查参数个数

void checkMaxSize(const std::vector<ValuePtr> &params, int size, const std::string &name);//用于检查参数个数

void throwTypeError(const std::string &name, const std::string &expectedType);//用于输出格式错误
...
ValuePtr quoteForm(const std::vector<ValuePtr> &params, EvalEnv &env) {//传入一个size为1的vector，输出其中唯一的元素转为quote之后的结果
...
ValuePtr lambdaForm(const std::vector<ValuePtr> &params, EvalEnv &env) {
    ...
    return std::make_shared<LambdaValue>(paramNamesStrVector, std::vector<ValuePtr>(params.begin() + 1, params.end()),
...
ValuePtr quasiquoteForm(const std::vector<ValuePtr> &params, EvalEnv &env) {
    ...
    if (params[0]->isPair()) {//如果是对子，需要判断中间的unquote
        ...
        return std::make_shared<PairValue>(quasiquoteForm({pairvalue->getCar()}, env),
                                           quasiquoteForm({pairvalue->getCdr()}, env));//递归调用quasiquote
    }
